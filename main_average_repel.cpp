...
//Then, the user and movie rating are repelled away from an updated average user and movie rating vector, independently.
...
void moveVectors(double *user, double *mr, double movieRating, double **movieRatingsArr, double etaInitial, double userEta, double mrEta, double *randomUser1, double *randomMR1, int dimensions, double z);
...
int main(int argc, char *argv[]) {
	...
	//Initialize arrays for averages for repulsion
	int *userRepulsionSample = new int[zSampleSize];
	int *mrRepulsionSample = new int[zSampleSize];
	double *averageUser = new double[dimensions];
	double *averageMR = new double[dimensions];

	//Fill the arrays with zero
	fill_n(averageUser, dimensions, 0);
	fill_n(averageMR, dimensions, 0);

	//Generate sample of users
	random_shuffle(dataIndexes.begin(), dataIndexes.end());
	for (int i1 = 0; i1 < zSampleSize; i1++) {
		userRepulsionSample[i1] = dataIndexes[i1];

		int *dataPt = data[dataIndexes[i1]];
		int userId = dataPt[USER_ID_IDX];

		double *userVector = userVectors[userId - 1];
		for (int dimension = 0; dimension < dimensions; dimension++) {
			double pt = userVector[dimension];
			averageUser[dimension] += pt;
		}
	}

	//Generate sample of movie ratings
	random_shuffle(dataIndexes.begin(), dataIndexes.end());
	for (int i1 = 0; i1 < zSampleSize; i1++) {
		mrRepulsionSample[i1] = dataIndexes[i1];
		...
		for (int dimension = 0; dimension < dimensions; dimension++) {
			...
			averageMR[dimension] += pt;
		}
	}

	for (int dimension = 0; dimension < dimensions; dimension++) {
		averageUser[dimension] /= zSampleSize;
		averageMR[dimension] /= zSampleSize;
	}
	...
	for (int iteration = 0; iteration < iterations; iteration++) {
		...
		for (int dataIdx = 0; dataIdx < numDataPoints; dataIdx++) {
			...
			//Get random new vectors to update z and repulsion with
			...
			//Get the oldest user vector from the repulsion sample
			...
			//Get the oldest mr vector from the repulsion sample
			int oldestMRDataIdx = mrRepulsionSample[oldestSampleIdx];
			int oldestMovieId = data[oldestMRDataIdx][MOVIE_ID_IDX];
			int oldestMovieRating = data[oldestMRDataIdx][MOVIE_RATING_IDX];
			double *oldestMRVector = movieRatingVectors[oldestMovieId - 1][oldestMovieRating - 1];

			//Update the averages
			for (int dimension = 0; dimension < dimensions; dimension++) {
				double oldestUserComponent = oldestUserVector[dimension];
				double newestUserComponent = randomUserVec[dimension];
				averageUser[dimension] += (newestUserComponent - oldestUserComponent) / zSampleSize;
				
				double oldestMRComponent = oldestMRVector[dimension];
				double newestMRComponent = randomMRVec[dimension];
				averageMR[dimension] += (newestMRComponent - oldestMRComponent) / zSampleSize;
			}
			...
			moveVectors(userVec, movieRatingVec, movieRating, movieRatingsArr, eta, userEta, mrEta, averageUser, averageMR, dimensions, initialZ);
